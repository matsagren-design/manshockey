...
const configs = {
  matches: {
    empty: { opponent:'', game_date:'', home_away:'Hemma', arena:'', city:'', tv_link:'', result:'', report_before:'', report_after:'' },
    titleField: 'opponent',
    tagField: 'home_away'
  },
  scout: {
    empty: { match_id:'', category:'', score:80, note:'' },
    titleField: 'category',
    tagField: 'score'
  },
  media: {
    empty: { title:'', source:'', url:'', tag:'', summary:'', published_at:'' },
    titleField: 'title',
    tagField: 'tag'
  },
  travel: {
    empty: { origin:'ARN', destination:'YYC', airline:'', max_price_sek:10000, depart_after:'09:30', avoid_usa:1, note:'' },
    titleField: 'airline',
    tagField: 'destination'
  },
  documents: {
    empty: { title:'', category:'', note:'', file_key:'', url:'' },
    titleField: 'title',
    tagField: 'category'
  }
};

export function DataPage({ type, title, kicker, items, setItems, isAdmin, reload }) {
  ...
  function startCreate() {
    ...
    setTimeout(() => document.getElementById(type+'-form')?.scrollIntoView({behavior:'smooth'}), 50);
  }
  ...
  async function submit(e) {
    ...
    if (editing) {
      ...
      const res = await updateItem(type, updated);
      setMessage(res.ok ? 'Uppdaterad i D1.' : 'Uppdaterad lokalt, men D1 svarade med fel.');
    } else {
      const temp = { id: Date.now(), ...form };
      setItems(prev => [temp, ...prev]);
      const res = await createItem(type, form);
      setMessage(res.ok ? 'Sparad i D1.' : 'Sparad lokalt, men D1 svarade med fel.');
    }
    setEditing(null);
    setForm(config.empty);
  ...
  async function remove(item) {
    ...
    setMessage(res.ok ? 'Borttagen från D1.' : 'Borttagen lokalt, men D1 svarade med fel.');
  }
    ...
    <div className="grid">
      {items.map((item, idx) => <article className="tile" key={item.id || idx}>
        ...
        {isAdmin && <div className="row-actions">
          <button onClick={() => startEdit(item)}><Edit3 size={15}/> Redigera</button>
          <button className="danger" onClick={() => remove(item)}><Trash2 size={15}/> Ta bort</button>
        </div>}
      </article>)}
    </div>
    {isAdmin && <form id={type+'-form'} className="admin-form inline-form" onSubmit={submit}>
      <div className="form-head">
        <h2>{editing ? 'Redigera post' : 'Ny post'}</h2>
        {editing && <button type="button" className="ghost" onClick={() => {setEditing(null); setForm(config.empty)}}><X size={16}/> Avbryt</button>}
      </div>
