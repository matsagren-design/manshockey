...
import{
  CheckCircle2,Clock,ExternalLink,FileSearch,Filter,Globe2,
  Newspaper,RefreshCw,Search,ShieldCheck,Video,XCircle,Archive,Layers3
}from'lucide-react';
...
function intelligenceLabel(category){
  if(category==='player')return 'Måns';
  if(category==='team')return 'Brooks';
  if(category==='league')return 'BCHL';
  if(category==='hockey')return 'Hockey';
  if(category==='wrong_person')return 'Fel person';
  return 'Övrigt';
}
  ...
  return <article className="tile media-watch-card">
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'flex-start'}}>
      <div style={{display:'flex',gap:7,flexWrap:'wrap'}}>
        ...
        <span className="tag">{intelligenceLabel(item.intelligence_category)}</span>
        {item.status==='history'&&<span className="tag">Historik</span>}
        {item.status==='background'&&<span className="tag">Bakgrund</span>}
      </div>
      <span style={{fontSize:12,opacity:.72,whiteSpace:'nowrap'}}>
        Relevans {Math.round(Number(item.calculated_relevance??item.relevance_score??0))}/100
      </span>
    </div>
    ...
    {item.snippet&&<p style={{opacity:.82,display:'-webkit-box',WebkitLineClamp:5,WebkitBoxOrient:'vertical',overflow:'hidden'}}>
      {item.snippet.length>800?`${item.snippet.slice(0,800)}…`:item.snippet}
    </p>}
...
function SummaryCard({icon,title,value}){
  return <article className="tile" style={{minHeight:120}}>
    <div style={{opacity:.72}}>{icon}</div>
    <h3 style={{fontSize:28,margin:'12px 0 4px'}}>{value}</h3>
    <p style={{margin:0,opacity:.72}}>{title}</p>
  </article>;
}
...
export function Media({media=[]}){
  ...
  async function runSearch(){
    try{
      ...
      setTab('inbox');
    }catch(e){setError(`Nätsökningen misslyckades: ${e.message||e}`)}
  ...
  async function runCleanup(){
    ...
    try{
      ...
      const r=await fetch('/api/media-watch',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'cleanup'})});
      const d=await r.json();
      if(!r.ok||!d.ok)throw new Error(d?.error||`HTTP ${r.status}`);
      setItems(Array.isArray(d.items)?d.items:[]);
      if(d.summary)setSummary(d.summary);
      ...
      const c=d.cleanup||{};
      setNotice(
        `Sorteringen klar: ${c.scanned??0} kontrollerade · ${c.movedToInbox??0} till Aktuellt · ${c.movedToHistory??0} till Historik · ${c.movedToBackground??0} till Bakgrund · ${c.autoIrrelevant??0} till Irrelevant.`
      );
      setTab('inbox');
    }catch(e){setError(`Sorteringen misslyckades: ${e.message||e}`)}
  ...
  },[items,tab,typeFilter,categoryFilter,textFilter]);
  ...
  return <Page kicker="Media Intelligence" title="Media och nyheter" action={
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:12,marginBottom:22}}>
      ...
      <SummaryCard icon={<Archive size={20}/>} title="Historik" value={summary.history||0}/>
      <SummaryCard icon={<Layers3 size={20}/>} title="Bakgrund" value={summary.background||0}/>
    ...
    {notice&&<div className="tile" style={{marginBottom:16}}>
      <CheckCircle2 size={18}/><strong>{notice}</strong>
      {lastSearch?.creditsUsed!==undefined&&<small style={{display:'block',opacity:.65,marginTop:6}}>Firecrawl credits: {lastSearch.creditsUsed}</small>}
      {lastCleanup&&<small style={{display:'block',opacity:.65,marginTop:6}}>Skyddade godkända: {lastCleanup.approvedProtected??0}</small>}
    </div>}
