...
import { Bot, CalendarDays, ExternalLink, FileText, MapPin, Newspaper, Plane, Save, Target, Trophy, Video } from 'lucide-react';
import { Page } from '../components/Layout.jsx';
import { formatDate, isSameMatch, updateItem } from '../lib/api.js';

function Section({ title, icon, children }) {
  return <section className="mc-section"><h2>{icon}{title}</h2>{children}</section>
}

export function MatchCenter({ matches, scout, media, travel, documents, selectedMatchId, setSelectedMatchId, user, reload }) {
  const selected = useMemo(() => matches.find(m => String(m.id) === String(selectedMatchId)) || matches[0], [matches, selectedMatchId]);
  const matchScout = scout.filter(x => isSameMatch(x, selected?.id));
  const matchMedia = media.filter(x => isSameMatch(x, selected?.id));
  const matchTravel = travel.filter(x => isSameMatch(x, selected?.id));
  const matchDocs = documents.filter(x => isSameMatch(x, selected?.id));
  const [draft, setDraft] = useState(selected || {});
  const [message, setMessage] = useState('');

  React.useEffect(() => setDraft(selected || {}), [selected?.id]);

  async function saveReport() {
    const res = await updateItem('matches', draft);
    setMessage(res.ok ? 'Matchcenter uppdaterat.' : (res.error || 'Kunde inte spara.'));
  ...
  return <Page kicker="Enterprise Matchcenter" title={`Brooks ${selected.home_away === 'Hemma' ? 'vs' : '@'} ${selected.opponent}`} action={
    ...
    <div className="match-layout">
      <div className="match-main">
        <div className="match-hero">
          ...
          <div className="match-actions">
            {selected.tv_link && <a href={selected.tv_link} target="_blank" rel="noreferrer"><Video size={16}/> TV / FloHockey</a>}
            {selected.map_url && <a href={selected.map_url} target="_blank" rel="noreferrer"><MapPin size={16}/> Karta</a>}
          </div>
        </div>

        <Section title="Rapporter" icon={<FileText/>}>
          <div className="report-grid">
            <div><h3>Inför match</h3><p>{selected.report_before || 'Ingen inför-rapport ännu.'}</p></div>
            <div><h3>Efter match</h3><p>{selected.report_after || 'Efterrapport fylls i efter match.'}</p></div>
          </div>
        </Section>
      ...
      <aside className="match-side">
        <Section title="Resultat" icon={<Trophy/>}>
          <p>{selected.result || 'Resultat saknas'}</p>
          <div className="score-row"><strong>{selected.brooks_goals ?? '-'}</strong><span>Brooks</span><strong>{selected.opponent_goals ?? '-'}</strong><span>{selected.opponent}</span></div>
        </Section>
        <Section title="Resor" icon={<Plane/>}>
          {matchTravel.length ? matchTravel.map(t => <p key={t.id}><b>{t.airline}</b>: {t.origin} → {t.destination}, {t.status}</p>) : <p>Ingen resa kopplad.</p>}
        </Section>
        <Section title="Dokument" icon={<FileText/>}>
        ...
        {user?.role === 'admin' && <Section title="Snabbredigera match" icon={<Save/>}>
          ...
          <label>Inför<textarea value={draft.report_before || ''} onChange={e => setDraft({...draft, report_before:e.target.value})}/></label>
          <label>Efter<textarea value={draft.report_after || ''} onChange={e => setDraft({...draft, report_after:e.target.value})}/></label>
          <label>AI-summary<textarea value={draft.ai_summary || ''} onChange={e => setDraft({...draft, ai_summary:e.target.value})}/></label>
          <button onClick={saveReport}><Save size={16}/> Spara match</button>
        </Section>}
      </aside>
    </div>
  </Page>
