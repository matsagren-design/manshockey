...
async function request(resource, method, item) {
  try {
    const response = await fetch(`/api/${resource}`, { method, headers:{'Content-Type':'application/json'}, body:JSON.stringify(item) });
    return await response.json();
  } catch { return { ok:false }; }
}
...
export async function me() {
  return getJson('/api/auth/me', { ok:false, user:null });
}
